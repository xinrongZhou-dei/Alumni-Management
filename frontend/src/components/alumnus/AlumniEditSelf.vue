<template>
  <div class="alumni-edit">
    <el-button type="text" @click="goMain" class="back-btn">
    ...
    <el-card class="edit-card">
      ...
      <el-form @submit.prevent="onSubmit" class="edit-form" size="large">
        <div class="form-section">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              ...
              <div :class="['info-value', form.region ? 'info-value-filled' : 'info-value-empty']"><el-select v-model="form.region" placeholder="请选择" class="full-width"><el-option v-for="opt in regionOptions" :key="opt.value" :label="opt.label" :value="opt.value" /></el-select></div>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <div :class="['info-value', form.maritalStatus ? 'info-value-filled' : 'info-value-empty']"><el-select v-model="form.maritalStatus" placeholder="请选择" class="full-width"><el-option v-for="opt in maritalOptions" :key="opt.value" :label="opt.label" :value="opt.value" /></el-select></div>
            </div>
            <div class="info-item">
        ...
        <div class="form-section">
          ...
          <div class="section-tip">注：校友名录中将展示您的第一条教育经历，如需展示最新教育，请将其拖动到最前面。</div>
                ...
                <el-row :gutter="20">
                  ...
                  <el-col :span="7">
                    <el-form-item label="毕业年份">
                      <el-input v-model="edu.graduationYear" type="number" />
                    </el-form-item>
                  </el-col>
                </el-row>
                <el-row :gutter="20">
        ...
        <div class="form-section">
          ...
          <div class="section-tip">注：校友名录中将展示您的第一条工作经历，如需展示最新工作，请将其拖动到最前面。</div>
        ...
        <div class="form-actions">
          <el-button @click="goMain">取消</el-button>
...
onMounted(async () => {
  ...
  try {
    ...
    alumniId = userInfo.alumniId || userInfo.alumniID || userInfo.alumni_id || userInfo.id || userInfo.alumniId || ''
    if (!alumniId && userInfo) {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      form.value = { ...alumniDefault, ...(res.data.data.alumni || {}) }
      ...
      tertiaryList.value = Array.isArray(res.data.data.tertiary_information) ? res.data.data.tertiary_information : []
      careerList.value = Array.isArray(res.data.data.carrer_information) ? res.data.data.carrer_information : []
    }
  } finally {
    loading.value = false
  }
...
async function onSubmit() {
  try {
    // 构造alumniPayload，所有字段都传递
    const alumniPayload = {
      alumni_id: String(form.value.alumniId),
      ...
      region: form.value.region || '大陆',
      birthday: form.value.birthday
    }
    // 需要把被标记deleted且有id的项单独收集，传给后端
    const deletedTertiaryIds = tertiaryList.value.filter(e => e.deleted && e.id).map(e => e.id)
    const deletedCareerIds = careerList.value.filter(w => w.deleted && w.id).map(w => w.id)
    const payload = {
      ...
      delete_tertiary_ids: deletedTertiaryIds,
      delete_career_ids: deletedCareerIds
    }
    const res = await axios.put('/api/register/alumni-update', payload, {
      headers: { Authorization: sessionStorage.getItem('alumni_token') }
    })
    if (res.data.code === 0) {
      // 如果有新token且关键数据不为空，刷新本地token和user_info
      if (res.data.data && res.data.data.token && res.data.data.chineseName && res.data.data.alumni_id) {
        if (isValidJWT(res.data.data.token)) {
          sessionStorage.setItem('alumni_token', res.data.data.token)
        }
        sessionStorage.setItem('user_info', JSON.stringify({
          chineseName: res.data.data.chineseName,
          affiliation: res.data.data.affiliation,
          role: res.data.data.role || 'alumni',
          alumniId: res.data.data.alumni_id
        }))
      } else if (res.data.data && ( !res.data.data.token || !res.data.data.chineseName || !res.data.data.alumni_id )) {
        // 关键数据缺失，强制登出
        sessionStorage.removeItem('alumni_token')
        sessionStorage.removeItem('user_info')
        window.alert('信息异常，请重新登录')
        router.push('/login')
        return
      }
      router.push('/') // 保存后返回校友端主页
